...
function readPersistentJson(key) {
  try {
    let raw;
    if (globalThis.$persistentStore?.read) raw = globalThis.$persistentStore.read(key);
    else if (globalThis.$prefs?.valueForKey) raw = globalThis.$prefs.valueForKey(key);
    if (!raw) return null;
    return typeof raw === "string" ? JSON.parse(raw) : raw;
  } catch {
    return null;
...
function writePersistentJson(key, value) {
  try {
    ...
    if (globalThis.$persistentStore?.write) return globalThis.$persistentStore.write(raw, key);
    if (globalThis.$prefs?.setValueForKey) return globalThis.$prefs.setValueForKey(raw, key);
  } catch {}
  return false;
}

function shadowrocketGet(options) {
  return new Promise((resolve, reject) => {
    if (!globalThis.$httpClient?.get) return reject(new Error("HTTP client unavailable"));
    globalThis.$httpClient.get(options, (error, response, body) => {
      if (error) return reject(new Error(String(error)));
      const status = Number(response?.status || response?.statusCode || 0);
      if (status < 200 || status >= 300) {
        const failure = new Error(`remote config HTTP ${status}`);
        failure.status = status;
        return reject(failure);
      }
      try { resolve(JSON.parse(body)); } catch (parseError) { reject(parseError); }
    });
  });
}

export async function loadRemoteConfig(args, options = {}) {
  const deviceId = String(args.deviceId || "").trim();
  const deviceToken = String(args.deviceToken || "").trim();
  const baseUrl = String(args.configBaseUrl || "").trim().replace(/\/+$/, "");
  if (!deviceId || !deviceToken || !baseUrl) return null;
  const cacheKey = "wloc_remote_config";
  const readCache = options.readCache || (() => readPersistentJson(cacheKey));
  const writeCache = options.writeCache || ((value) => writePersistentJson(cacheKey, value));
  const request = options.httpGet || shadowrocketGet;
  try {
    const config = await request({
      url: `${baseUrl}/api/client/${encodeURIComponent(deviceId)}/config`,
      headers: { Authorization: `Bearer ${deviceToken}`, Accept: "application/json" },
      timeout: 3,
    });
    resolveConfig({}, config);
    writeCache({ deviceId, config, updatedAt: Date.now() });
    return config;
  } catch (error) {
    ...
    if (cached?.deviceId === deviceId && cached.config) {
      resolveConfig({}, cached.config);
...
async function run() {
  ...
  const args = parseArguments(globalThis.$argument);
  const saved = readSavedSettings();
  const remote = await loadRemoteConfig(args);
  const config = resolveConfig(resolveConfig(args, saved), remote);
  const output = await processResponse(response, config, { savedConfigPresent: saved != null || remote != null });
  done(output, true);
}
